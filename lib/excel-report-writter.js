...
function writeExcelReport(fileName, result) {
    ...
    detailsSheet.set(colIndx++, rowIndx, 'Labels');
    detailsSheet.set(colIndx++, rowIndx, 'Components');
    ...
    _.forEach(result.details, (items, author) => {
        _.forEach(_.sortBy(items, ['started']), (item) => {
            ...
            detailsSheet.set(colIndx++, rowIndx, item.issue.fields.labels.sort().join(', '));
            detailsSheet.set(colIndx++, rowIndx, item.issue.fields.components.map( _ => _.name).sort().join(', '));
