...
function WriteStringReportToHandler(options, writer) {
    ...
    `);
    writer(`<h3>Time-Report: ${options.dates.startDate.format('MMM dddd')} - ${options.dates.endDate.format('MMM dddd')}</h3>`);
    writer(`<span class="text-small">${options.dates.startDate.format('LLLL')} - ${options.dates.endDate.format('LLLL')}</span>`);
    writer('<table>');
    summary.map((item) => {
